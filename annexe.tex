...
As described in Chapter \ref{chap-simulation}, a standard Kalman filter was tuned to minimize the RMSE of the output estimates using 300 data samples from the $2\times2$ linear system. Since the system is symmetrical and the two RODDs have the same parameters, it was assumed that the two parameters, $\sigma_{w_p,1,\text{opt}}$ and $\sigma_{w_p,2,\text{opt}}$ must be identical. This simplified the search process since only one optimum value needed to be found.
...
\begin{figure}[htp]
	...
	\caption{Sensitivity of Kalman filter tuning}
...
\begin{figure}[htp]
	...
	\caption{Tuning of a Kalman filter to minimise estimation errors - $2\times2$ system}
