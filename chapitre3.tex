...
In this section, results of an experiment to test the observers using data from the grinding process simulator are presented. This work is an extension of work published at a recent conference. %\citep{}.

For the purposes of this experiment, the size of the ground product (P80) leaving the cyclone overflow is sampled every 3 minutes and a zero-mean Gaussian measurement noise with a standard deviation of 5 $\mu\text{m}$ is added directly to the simulation output.

\subsection{System Identification} \label{section-sysid}

Model-based observers such as the Kalman filter require a dynamic model of the process. 
Here, a single-input, single-output (SISO) process model is considered with a RODD step disturbance at the input, as shown in Figure \ref{fig:obs_model}. The disturbance, $p(k)$, represents step changes in the ore mix factor described in Section \ref{section-simulator}. The output variable, $y(k)$, is the product P80. $v(k)$ is an additive measurement noise. 
%For simplicity, no manipulatable variables or other measured variables are considered although the approach is not limited to SISO systems.

\begin{figure}[htp]
	\centering
	\includegraphics[width=8.5cm]{images/obs-model-diag.pdf}
