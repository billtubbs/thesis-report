...
The two random shocks were both simulated with the same parameters as in the previous SISO simulation, $\epsilon_i=0.01, \sigma_{w_{p,i}}=0.01$, and $b_i=100$ . However, the standard deviations of the measurement noises, $\sigma_{M,1}$ and $\sigma_{M,2}$, were $0.2$, twice as large as in the SISO simulations, and the sampling period, \gls{Ts}, was 1, twice as long.

Figure \ref{fig:rod-obs-sim-2-ioplot} shows the first 600 input-output data samples from a simulation of the system with a total duration of 2500 (5000 samples). The lower of the two plots shows the input signals, $u_1(k)$, $u_2(k)$, $p_1(k)$, and $p_2(k)$. From this it can be seen that a large shock occurred to $p_1(k)$ at $t=122$ and 6 random shocks occurred to $p_2(k)$ of various magnitudes. In this simulation, there were no changes to the known input variables. The upper plot shows the simulated output measurements, $y_1(k)$ and $y_2(k)$. No input disturbances other than $w_{p,1}(k)$ and $w_{p,2}(k)$ were simulated (i.e. $w_1(k)=w_2(k)=0$).
