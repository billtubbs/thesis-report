...
Because these processes are stochastic, it is important to run simulations of sufficient duration to be confident that the results are representative of the expected performance of the observers. Figure \ref{fig:rod-obs-sim1-yest-all-seed-RMSE-box} provides an indication of the sensitivity of the \gls{RMSE} values to the initialization of the pseudo-random number generator used in these simulations. This figure shows the results of 10 simulations of 5000 samples each, carried out with different initializations. From these results it can be seen that the \gls{RMSE} values are somewhat sensitive to the random initialization but that the differences between the two multi-model observers and the Kalman filter are significant and unlikely to be a result of the random initialization. Therefore, these simulations of duration 5000 sample periods may be considered sufficient for the purposes of evaluating the relative performance of these observers.
\begin{figure}[htp]
	\centering
...
Figure \ref{fig:rod-obs-sim1-yest-all-seed-RMSE-box} shows that the multi-model observers are a significant improvement on the best single Kalman filter. However, there is very little difference between the two in terms of RMSE. The performance of the two multiple-model observers is close to that of the scheduled Kalman filter, which is the best that could be achieved given the true shock sequence, $\Gamma(k)$.
...
\cite{robertson_method_1998} stated that in the case of more than one RODD disturbance, it often takes many sampling intervals to discriminate among the possible disturbances. The main motivation for the detection intervals used in their approach is to achieve a longer \textit{detection horizon} without requiring a very large number of hypotheses. To evaluate this particular problem, a \textit{multiple-input, multiple output} (MIMO) system with a 2-input, 2-output ($2\times2$) process model was chosen. The system has two independent \gls{RODD} step disturbances added to each manipulated input, as shown in the functional diagram in Figure \ref{fig:sim-sys-diag-2x2}. 
...
\begin{equation} \label{eq:sim-sys-siso--ct}
	...
	\end{array}\right]
\end{equation}
where $s$ is the Laplace variable.  This model was chosen because the dynamics of each input-output combination have the same time constants. The only differences are in the magnitude of the static gains. The model was converted to a discrete-time model with a sampling period, \gls{Ts} of 1. The discrete-time state space model used in the observers to simulate the augmented system including disturbance models was
...
The two random shocks were both simulated with the same parameters as in the SISO simulation, $\epsilon_i=0.01, \sigma_{w_{p,i}}=0.01$, and $b_i=100$ . However, the standard deviations of the measurement noises, $\sigma_{M,1}$ and $\sigma_{M,2}$, were $0.2$, twice as large as in the SISO simulations, and the sampling period was twice as long.

Figure \ref{fig:rod-obs-sim-2-ioplot} shows the first 300 input-output data samples from a simulation of the system with a total duration of 5000. The lower of the two plots shows the input signals, $u_1(k)$, $u_2(k)$, $p_1(k)$, and $p_2(k)$. In this simulation, there were no changes to the known input variables. The upper plot shows the simulated output measurements, $y_1(k)$ and $y_2(k)$. No input disturbances other than $w_{p,1}(k)$ and $w_{p,2}(k)$ were simulated (i.e. $w_1(k)=w_2(k)=0$).

As in the previous experiment, a standard Kalman filter was tuned experimentally to achieve a low \gls{RMSE} of the output estimates. The details of this tuning process are included in Annex \ref{section:annex-sim-2-KF-tuning} including the chosen value of $\mathbf{Q}_{\text{KF3}}$ (\ref{eq:sim-sys-sim3-KF3-Q}). 
...
%	\caption{Cumulative errors of multi-model observer output estimates –  $2\t_y2mes2$ system}
...
\begin{table}[hb]
	\begin{center}
		...
		% See: https://texblog.org/2019/06/03/control-the-width-of-table-columns-tabular-in-latex/
		\begin{tabular}{p{0.16\textwidth}>{\centering\arraybackslash}p{0.11\textwidth}>{\centering\arraybackslash}p{0.16\textwidth}>{\centering\arraybackslash}p{0.16\textwidth}>{\centering\arraybackslash}p{0.16\textwidth}}
			...
			Sub-optimal algorithm & - & Sequence fusion 95 & Sequence fusion & Sequence pruning \\
			\hline
			Parameters &  &  & &  \\
