...
% Specify system case
p_case = 1;  % 1st order SISO system with RODD step input disturbance
...
filename = sprintf('rod_obs_sim2_%d_%d.csv', p_case, test);
...
% Check plant output with the model
[Y_model, ~, X_model] = lsim(Gpss, [U Wp], t);
assert(max(abs(X - X_model), [], [1 2]) < 1e-6)
assert(max(abs(Y - Y_model)) < 1e-6)

% Observer state estimates
X_est = sim_data{:, {'X_est_1', 'X_est_2', 'X_est_3', 'X_est_4', 'X_est_5', 'X_est_6'}};

% Observer output estimates
Y_est = sim_data{:, {'Y_est_1', 'Y_est_2', 'Y_est_3'}};
...
for i = 1:ny
    axs(i) = subplot(ny+n_dist,1,i);
    labels = cell(1, n_obs);
    for j = 1:n_obs
        plot(t, Y_est(:, ny*(j-1)+i),'Linewidth', 2); hold on;
        labels{j} = sprintf("$%s$ by %s", y_est_plot_labels{i}, obs_labels{j});
    end
    stairs(t, Y_m(:, i), 'k.')
    ylim(axes_limits_with_margin([Y_m Y_est], 0.1))
    set(gca, 'TickLabelInterpreter', 'latex')
    xlabel('$t$', 'Interpreter', 'Latex')
    y_label = string2latex(strjoin([y_labels(i) y_est_plot_labels(i)], ', '));
    ylabel(y_label, 'Interpreter','Latex')
    legend([labels string2latex(y_labels(i))], 'Interpreter','Latex', 'Location', 'best')
...
for i = 1:n_dist
    ...
    labels = cell(1, n_obs);
    for j = 1:n_obs
        plot(t, X_est(:, idx(i) + n*(j-1)), 'Linewidth', 2); hold on
        labels{j} = sprintf("$%s$ by %s", x_est_plot_labels{idx(i)}, obs_labels{j});
    end
    stairs(t, Pd(:, i), 'k-'); hold on;
    %ylim(axes_limits_with_margin(X_est, 0.1))
    set(gca, 'TickLabelInterpreter', 'latex')
    xlabel('$t$', 'Interpreter', 'Latex')
...
filename = sprintf('rod-obs-sim-1-%d-est-KF', test);
saveas(gcf, fullfile(plot_dir, strcat(filename, '.png')))
save_fig_to_pdf(fullfile(plot_dir, strcat(filename, '.pdf')))


%% Cumulative output error estimates plot

% Index of state corresponding to input disturbance
idx = 2;
...
for i = 1:n_dist
    ...
    filename = sprintf('rod-obs-sim-1-%d_est_err', test);
    saveas(gcf, fullfile(plot_dir, strcat(filename, '.png')))
    save_fig_to_pdf(fullfile(plot_dir, strcat(filename, '.pdf')))
